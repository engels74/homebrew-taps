...
# Resolver: qView.
#
# Reads /releases/latest and requires the exact asset qView-<version>.dmg at the
# exact expected URL. The exact match deliberately excludes qView-<version>-legacy.dmg
# and non-macOS builds.
...
# qView tags are "7.1" or "7.1.1".
tag_re='^v?[0-9]+\.[0-9]+(\.[0-9]+)?$'
[[ "${tag}" =~ ${tag_re} ]] || die "Unexpected upstream tag format: ${tag}"

version="${tag#v}"
expected_name="qView-${version}.dmg"
